...
from utils.tools import EarlyStopping, adjust_learning_rate, save_model_ETT, load_model_ETT, plot_line,visual
...
from models.PatchFusionMLP import TFMRN
    ...
    def test(self, setting, evaluate=False):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pbar):
            ...
            if i % 1 == 0:
                ...
                gt = np.concatenate((input[0, :, -1], true1[0, :, -1]), axis=0)
                pd = np.concatenate((input[0, :, -1], pred1[0, :, -1]), axis=0)
